...
async def get_realtime_market_data(tickers: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        data = await asyncio.to_thread(yf.download, tickers_str, period="1d", interval="1m", progress=False, group_by='ticker')
        ...
        if len(tickers) == 1:
            ...
            if not data.empty:
                latest = data.iloc[-1]
                # Check if 'Close' is a scalar or series (yfinance versions vary)
                try:
                ...
                result[ticker] = {
                    ...
                    "change_percent": 0.0, # Hard to calc real-time change without prev close easily accessible in this view
